...
logo = Image.open("logo.png")  # Make sure your logo file is named 'logo.png' and in the same folder
...
with st.expander("📋 Enter Player Names and Handicaps", expanded=True):
    ...
    with col1:
        ...
        a1_hcp = st.number_input("Handicap for Player 1", min_value=0, max_value=54, value=0, key="a1")
    with col2:
        a2_name = st.text_input("Enter name for Team A - Player 2", placeholder="e.g. Sarah")
        a2_hcp = st.number_input("Handicap for Player 2", min_value=0, max_value=54, value=0, key="a2")

    st.markdown("### Team B")
    col3, col4 = st.columns(2)
    with col3:
        b1_name = st.text_input("Enter name for Team B - Player 1", placeholder="e.g. Mike")
        b1_hcp = st.number_input("Handicap for Player 1", min_value=0, max_value=54, value=0, key="b1")
    with col4:
        b2_name = st.text_input("Enter name for Team B - Player 2", placeholder="e.g. Emma")
        b2_hcp = st.number_input("Handicap for Player 2", min_value=0, max_value=54, value=0, key="b2")
